...
: CBaseVideoRenderer(__uuidof(CLSID_TextureRenderer), L"Texture Renderer", pUnk, phr)
...
HRESULT CTextureRenderer::SetMediaType(const CMediaType *pmt)
{
	...
	if (mTextureFormat != D3DFMT_X8R8G8B8 &&
		mTextureFormat != D3DFMT_A1R5G5B5) {
			Msg(TEXT("Texture is format we can't handle! Format = 0x%x"), mTextureFormat);
...
HRESULT CTextureRenderer::DoRenderSample( IMediaSample * pSample )
{
	...
	if (mTextureFormat == D3DFMT_X8R8G8B8)
	{
		...
		{
			pdwSource = ( DWORD*)pBmpBuffer;
			pdwDest = ( DWORD*)pTxtBuffer;

			for( col = 0; col < dwordWidth; col ++ )
			{
				pdwDest[0] =  pdwSource[0] | 0xFF000000;
				pdwDest[1] = ((pdwSource[1]<<8)  | 0xFF000000) | (pdwSource[0]>>24);
				pdwDest[2] = ((pdwSource[2]<<16) | 0xFF000000) | (pdwSource[1]>>16);
				pdwDest[3] = 0xFF000000 | (pdwSource[2]>>8);
				pdwDest +=4;
				pdwSource +=3;
			}

			// we might have remaining (misaligned) bytes here
			pbSource = (BYTE*) pdwSource;
	...
	if (mTextureFormat == D3DFMT_A1R5G5B5)
	{
