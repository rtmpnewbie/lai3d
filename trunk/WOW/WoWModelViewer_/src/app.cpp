...
void WowModelViewApp::LoadSettings()
{
    ...
    wxFileConfig *pConfig = new wxFileConfig("Global", wxEmptyString, cfgPath,
    ...
    if(mpqArchives.GetCount() == 0)
    {
        ...
        {
            _T("enUS"), _T("koKR"), _T("frFR"), _T("deDE"), _T("zhCN"), _T(
                "zhTW"), _T("esES"), _T("ruRU")
        };
        const wxString locales2[] = 
        {
            _T("enGB")
        };

#ifdef WotLK
        const wxString defaultArchives[] = 
        {
            _T("Patch-2.MPQ"), _T("Patch.MPQ"), _T("Lichking.MPQ"), 
			_T("Expansion.MPQ"), _T("Common-2.MPQ"), _T("Common.MPQ")
        };
        const wxString localeArchives[] = 
        {
            _T("Patch"), _T("Lichking-Locale"), _T("expansion-locale"), 
			_T("Locale"), _T("Base")
        };
#else 
        const wxString defaultArchives[] = 
        {
            _T("Patch-2.MPQ"), _T("Patch.MPQ"), _T("Expansion.MPQ"), _T(
                "Common.MPQ")
        };
        const wxString localeArchives[] = 
        {
            _T("Patch"), _T("expansion-locale"), _T("Locale"), _T("Base")
        };
#endif 
    ...
    if(langID ==  - 1)
    {
        ...
        wxCOMPILE_TIME_ASSERT(WXSIZEOF(langNames) == WXSIZEOF(langIds),
            LangArraysMismatch);
        langID = wxGetSingleChoiceIndex(_T("Please select a language:"), _T(
            "Language"), WXSIZEOF(langNames), langNames);
