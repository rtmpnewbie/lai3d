...
BEGIN_EVENT_TABLE(ModelControl, wxWindow)EVT_CHECKLISTBOX(ID_MODEL_GEOSETS,
    ModelControl::OnList)

EVT_COMBOBOX(ID_MODEL_NAME, ModelControl::OnCombo)EVT_COMBOBOX(ID_MODEL_LOD,
    ModelControl::OnCombo)

EVT_CHECKBOX(ID_MODEL_BONES, ModelControl::OnCheck)EVT_CHECKBOX(ID_MODEL_BOUNDS,
    ModelControl::OnCheck)EVT_CHECKBOX(ID_MODEL_RENDER, ModelControl::OnCheck)
    EVT_CHECKBOX(ID_MODEL_WIREFRAME, ModelControl::OnCheck)EVT_CHECKBOX
    (ID_MODEL_PARTICLES, ModelControl::OnCheck)EVT_CHECKBOX(ID_MODEL_TEXTURE,
    ModelControl::OnCheck)

EVT_COMMAND_SCROLL(ID_MODEL_ALPHA, ModelControl::OnSlider)EVT_COMMAND_SCROLL
    (ID_MODEL_SCALE, ModelControl::OnSlider)END_EVENT_TABLE()
...
ModelControl::ModelControl(wxWindow *parent, wxWindowID id)
{
    ...
    if(Create(parent, id, wxDefaultPosition, wxSize(160, 460), 0, _T(
        "ModelControlFrame") ) == false)
    {
        wxLogMessage(_T(
            "GUI Error: Failed to create a window for our ModelControl!") );
        return ;
    }

    try
    {
        modelname = new wxComboBox(this, ID_MODEL_NAME, _T(""), wxPoint(5, 5),
            wxSize(150, 16), 0, NULL, wxCB_READONLY);

        lblLod = new wxStaticText(this, wxID_ANY, _("View"), wxPoint(5, 25),
            wxDefaultSize);
        lod = new wxComboBox(this, ID_MODEL_LOD, _T(""), wxPoint(5, 40), wxSize
            (120, 16), 0, NULL, wxCB_READONLY, wxDefaultValidator, _T("LOD") );
            //|wxCB_SORT); //wxPoint(66,10)
        lod->Enable(false);

        lblAlpha = new wxStaticText(this, wxID_ANY, _("Alpha"), wxPoint(5, 65),
            wxDefaultSize);
        alpha = new wxSlider(this, ID_MODEL_ALPHA, 100, 0, 100, wxPoint(45, 65),
            wxSize(110, 30), wxSL_HORIZONTAL);

        lblScale = new wxStaticText(this, wxID_ANY, _("Scale"), wxPoint(5, 90),
            wxDefaultSize);
        scale = new wxSlider(this, ID_MODEL_SCALE, 100, 10, 300, wxPoint(45, 90)
            , wxSize(110, 30), wxSL_HORIZONTAL);

        bones = new wxCheckBox(this, ID_MODEL_BONES, _("Bones"), wxPoint(5, 115)
            , wxDefaultSize);
        box = new wxCheckBox(this, ID_MODEL_BOUNDS, _("Bounds"), wxPoint(5, 135)
            , wxDefaultSize);
        render = new wxCheckBox(this, ID_MODEL_RENDER, _("Render"), wxPoint(5,
            155), wxDefaultSize);
        wireframe = new wxCheckBox(this, ID_MODEL_WIREFRAME, _("Wireframe"),
            wxPoint(5, 175), wxDefaultSize);
        texture = new wxCheckBox(this, ID_MODEL_TEXTURE, _("Texture"), wxPoint
            (5, 195), wxDefaultSize);
        particles = new wxCheckBox(this, ID_MODEL_PARTICLES, _("Particles"),
            wxPoint(5, 215), wxDefaultSize);

        lblGeosets = new wxStaticText(this, wxID_ANY, _("Show Geosets"),
            wxPoint(5, 235), wxDefaultSize);
        geosets = new wxCheckListBox(this, ID_MODEL_GEOSETS, wxPoint(5, 250),
            wxSize(150, 120), 0, NULL, 0, wxDefaultValidator, _T("GeosetsList")
            );

        wxStaticText *lblXYZ = new wxStaticText(this, wxID_ANY, _T("X\nY\nZ"),
            wxPoint(2, 380), wxSize(20, 60) );
        txtX = new wxTextCtrl(this, ID_MODEL_X, _T("0.0"), wxPoint(25, 380),
            wxDefaultSize, wxTE_PROCESS_ENTER, wxDefaultValidator);
        txtY = new wxTextCtrl(this, ID_MODEL_Y, _T("0.0"), wxPoint(25, 400),
            wxDefaultSize, wxTE_PROCESS_ENTER, wxDefaultValidator);
        txtZ = new wxTextCtrl(this, ID_MODEL_Z, _T("0.0"), wxPoint(25, 420),
            wxDefaultSize, wxTE_PROCESS_ENTER, wxDefaultValidator);
...
void ModelControl::RefreshModel(Attachment *root)
{
    try
    {
        ...
        for(std::vector < Attachment * > ::iterator it = root->children.begin()
            ; it != root->children.end(); ++it)
        {
            ...
            for(std::vector < Attachment * > ::iterator it2 = (*it)
                ->children.begin(); it2 != (*it)->children.end(); ++it2)
            {
                ...
                for(std::vector < Attachment * > ::iterator it3 = (*it2)
                    ->children.begin(); it3 != (*it2)->children.end(); ++it3)
                {
        ...
        wxString tmp;
        modelname->Clear();
        for(std::vector < Attachment * > ::iterator it = attachments.begin();
    ...
    {
        wxLogMessage(_T(
            "Error: Problem occured in ModelControl::RefreshModel(Attachment *)") );
    }
...
void ModelControl::OnCombo(wxCommandEvent &event)
{
    ...
    if(id == ID_MODEL_LOD)
    {
        ...
        {
            wxLogMessage("ERROR - unable to open MPQFile: [%s]", model
                ->name.c_str() );
            f.close();
            return ;
        }
    ...
    else if(id == ID_MODEL_NAME)
    {
        ...
        if(modelname->GetSelection() < (int)attachments.size() )
        {
            ...
            model = static_cast < Model * > (attachments[modelname
                ->GetSelection()]->model);
