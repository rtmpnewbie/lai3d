...
IMPLEMENT_CLASS(ModelCanvas, wxWindow)BEGIN_EVENT_TABLE(ModelCanvas, wxWindow)
//IMPLEMENT_CLASS(ModelCanvas, wxGLCanvas)
//BEGIN_EVENT_TABLE(ModelCanvas, wxGLCanvas)
EVT_SIZE(ModelCanvas::OnSize)EVT_PAINT(ModelCanvas::OnPaint)
    EVT_ERASE_BACKGROUND(ModelCanvas::OnEraseBackground)

EVT_TIMER(ID_TIMER, ModelCanvas::OnTimer)EVT_MOUSE_EVENTS(ModelCanvas::OnMouse)
    EVT_KEY_DOWN(ModelCanvas::OnKey)END_EVENT_TABLE()


#ifdef _WIN32 
    // The following time related functions COULD be 64bit incompatible.
// for timeGetTime:
...
//: wxGLCanvas(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxNO_BORDER|wxCLIP_CHILDREN|wxFULL_REPAINT_ON_RESIZE, _T("ModelCanvas"), 0, wxNullPalette)
{
    ...
    if(!Create(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxNO_BORDER 
        | wxCLIP_CHILDREN | wxFULL_REPAINT_ON_RESIZE, _T("ModelCanvas") ) )
    {
        wxLogMessage(_T(
            "Critcal Error: Unable to create a window to handle our OpenGL rendering.\n\tWon't be able to continue.") );
        parent->Close();
        return ;
    }
    else
...
void ModelCanvas::OnMouse(wxMouseEvent &event)
{
    ...
    else if(model)
    {
        if(model->animManager)
        {
            mul *= model->animManager->GetSpeed();
        }
        //animSpeed;
        ...
        if(event.ButtonDown() )
        {
            ...
            if(bMouseLight)
                ...
                // going to use vRot to hold our temp light position (technically, our g_modelViewer->lightControl->lights rotation).
            {
                vRot0 = Vec3D(g_modelViewer->lightControl->GetCurrentPos().x,
                    g_modelViewer->lightControl->GetCurrentPos().y,
                    g_modelViewer->lightControl->GetCurrentPos().z);
            }
            else
            {
                vRot0 = model->rot;
...
inline void ModelCanvas::CreateTexture(wxString filename, GLuint texture)
{
    ...
    glTexImage2D(texFormat, 0, GL_RGBA8, image->GetWidth(), image->GetHeight(),
...
inline void ModelCanvas::RenderLight(Light *l)
{
    GLUquadricObj *quadratic = gluNewQuadric(); 
        // Storage For Our Quadratic Object & // Create A Pointer To The Quadric Object
    ...
    else if(l->type == LIGHT_POSITIONAL)
    {
        // Positional Light
...
inline void ModelCanvas::Render()
{
    ...
    else if(lightType == LIGHT_DYNAMIC)
    {
        ...
        {
            if(g_modelViewer->lightControl->lights[i].enabled && !g_modelViewer
                ->lightControl->lights[i].relative)
            {
                glLightfv(GL_LIGHT0 + i, GL_POSITION, g_modelViewer
                    ->lightControl->lights[i].pos);
    ...
    {
        glLightModelfv(GL_LIGHT_MODEL_AMBIENT, g_modelViewer->lightControl
            ->lights[0].diffuse); 
            // use diffuse, as thats our main 'colour setter'
    }
    // ==============================================
    ...
    if(model && (lightType == LIGHT_DYNAMIC) )
    {
        
            // Else, for all our models, we use the new "lighting control", IF we're not using model only lighting		
...
void ModelCanvas::ResetView()
{
    ...
    // items look better facing right by default
}
...
void ModelCanvas::LoadBackground(wxString filename)
{
    ...
    else
    {
        ...
        glTexImage2D(texFormat, 0, GL_RGBA8, image->GetWidth(), image
            ->GetHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE, buffer);
        drawBackground = true;
    }
...
void ModelCanvas::Zoom(float f, bool rel)
{
    if(rel)
    {
        ...
        model->pos.y += sin(model->rot.x *piover180) *sin(model->rot.y
            *piover180) *f;
...
void ModelCanvas::CheckMovement()
{
    ...
    if(wxGetKeyState(WXK_NUMPAD9) )
    // Rotate right
    {
        camera.RotateY( - 1.0f);
    }
    if(wxGetKeyState(WXK_NUMPAD5) )
    // Reset Camera
    {
        camera.Reset();
    }
    if(wxGetKeyState(WXK_NUMPAD4) )
    // Straff Left
    {
        camera.Strafe( - 0.05f);
    }
    ...
    if(!wmo)
    {
        speed = ( (timeGetTime() - lastTime) *model->animManager->GetSpeed() )
            / 7.0f;
    }
    else
    {
...
void ModelCanvas::Screenshot(const wxString fn, int x, int y)
{
    ...
    glReadPixels(0, 0, screenSize[2], screenSize[3], GL_BGRA_EXT,
    ...
    newImage->CreateFromArray(pixels, screenSize[2], screenSize[3], 32, 
        (screenSize[2] *4), false);
...
Attachment *Attachment::addChild(const char *modelfn, int id, int slot, float
    scale, float rot, Vec3D pos)
{
...
Attachment *Attachment::addChild(Displayable *disp, int id, int slot, float
    scale, float rot, Vec3D pos)
{
