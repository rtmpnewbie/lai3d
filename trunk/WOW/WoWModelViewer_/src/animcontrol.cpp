...
BEGIN_EVENT_TABLE(AnimControl, wxWindow)EVT_COMBOBOX(ID_ANIM, AnimControl
    ::OnAnim)EVT_COMBOBOX(ID_ANIM_SECONDARY, AnimControl::OnAnim)EVT_COMBOBOX
    (ID_ANIM_MOUTH, AnimControl::OnAnim)

EVT_COMBOBOX(ID_LOOPS, AnimControl::OnLoop)EVT_COMBOBOX(ID_SKIN, AnimControl
    ::OnSkin)EVT_COMBOBOX(ID_ITEMSET, AnimControl::OnItemSet)

EVT_CHECKBOX(ID_OLDSTYLE, AnimControl::OnCheck)EVT_CHECKBOX(ID_ANIM_LOCK,
    AnimControl::OnCheck)

EVT_BUTTON(ID_PLAY, AnimControl::OnButton)EVT_BUTTON(ID_PAUSE, AnimControl
    ::OnButton)EVT_BUTTON(ID_STOP, AnimControl::OnButton)EVT_BUTTON(ID_ADDANIM,
    AnimControl::OnButton)EVT_BUTTON(ID_CLEARANIM, AnimControl::OnButton)
    EVT_BUTTON(ID_PREVANIM, AnimControl::OnButton)EVT_BUTTON(ID_NEXTANIM,
    AnimControl::OnButton)

EVT_SLIDER(ID_SPEED, AnimControl::OnSliderUpdate)EVT_SLIDER(ID_SPEED_MOUTH,
    AnimControl::OnSliderUpdate)EVT_SLIDER(ID_FRAME, AnimControl
    ::OnSliderUpdate)END_EVENT_TABLE()

AnimControl::AnimControl(wxWindow *parent, wxWindowID id)
{
    wxLogMessage(_T("Creating Anim Control...") );

    if(Create(parent, id, wxDefaultPosition, wxSize(700, 90), 0, _T(
        "AnimControlFrame") ) == false)
    {
        wxMessageBox(_T("Failed to create a window for our AnimControl!"), _T(
            "Error") );
        wxLogMessage(_T(
            "GUI Error: Failed to create a window for our AnimControl!") );
        return ;
    }

    const wxString strLoops[10] = 
    {
        _T("0"), _T("1"), _T("2"), _T("3"), _T("4"), _T("5"), _T("6"), _T("7"),
            _T("8"), _T("9")
    };

    animList = new wxComboBox(this, ID_ANIM, _("Animation"), wxPoint(10, 10),
        wxSize(144, 16), 0, NULL, wxCB_READONLY | wxCB_SORT, wxDefaultValidator,
        _("Animation") ); //|wxCB_SORT); //wxPoint(66,10)
    animList2 = new wxComboBox(this, ID_ANIM_SECONDARY, _("Secondary"), wxPoint
        (154, 10), wxSize(144, 16), 0, NULL, wxCB_READONLY | wxCB_SORT,
        wxDefaultValidator, _("Secondary") ); //|wxCB_SORT); //wxPoint(66,10)
    animList2->Enable(false);
    animList2->Show(false);

    // Our hidden head/mouth related controls
    animList3 = new wxComboBox(this, ID_ANIM_MOUTH, _("Mouth"), wxPoint(10, 100)
        , wxSize(144, 16), 0, NULL, wxCB_READONLY | wxCB_SORT,
        wxDefaultValidator, _("Secondary") ); //|wxCB_SORT); //wxPoint(66,10)
    ...
    speedMouthSlider = new wxSlider(this, ID_SPEED_MOUTH, 10, 0, 40, wxPoint
        (255, 100), wxSize(100, 38), wxSL_AUTOTICKS);
    speedMouthSlider->SetTickFreq(10, 1);
    speedMouthSlider->Show(false);

    speedMouthLabel = new wxStaticText(this,  - 1, _("Speed: 1.0x"), wxPoint
        (180, 100), wxDefaultSize);
    speedMouthLabel->Show(false);
    // ---

    loopList = new wxComboBox(this, ID_LOOPS, wxString("0"), wxPoint(298, 10),
        wxSize(40, 16), 10, strLoops, wxCB_READONLY, wxDefaultValidator,
        wxString("Loops") ); //|wxCB_SORT); //wxPoint(66,10)
    btnAdd = new wxButton(this, ID_ADDANIM, _("Add"), wxPoint(338, 10), wxSize
        (36, 20) );

    skinList = new wxComboBox(this, ID_SKIN, _("Skin"), wxPoint(154, 10),
        wxSize(144, 16), 0, NULL, wxCB_READONLY);
    skinList->Show(FALSE);
    randomSkins = true;
    defaultDoodads = true;

    wmoList = new wxComboBox(this, ID_ITEMSET, _("Item set"), wxPoint(220, 10),
        wxSize(128, 16), 0, NULL, wxCB_READONLY);
    wmoList->Show(FALSE);
    wmoLabel = new wxStaticText(this,  - 1, _T(""), wxPoint(10, 15), wxSize(192,
        16) );
    wmoLabel->Show(FALSE);

    speedSlider = new wxSlider(this, ID_SPEED, 10, 1, 40, wxPoint(420, 56),
        wxSize(100, 38), wxSL_AUTOTICKS);
    speedSlider->SetTickFreq(10, 1);
    speedLabel = new wxStaticText(this,  - 1, _("Speed: 1.0x"), wxPoint(420, 40)
        , wxDefaultSize);

    frameLabel = new wxStaticText(this,  - 1, _("Frame: 0"), wxPoint(250, 40),
        wxDefaultSize);
    frameSlider = new wxSlider(this, ID_FRAME, 1, 1, 10, wxPoint(250, 56),
        wxSize(160, 38), wxSL_AUTOTICKS);
    frameSlider->SetTickFreq(2, 1);

    btnPlay = new wxButton(this, ID_PLAY, _("Play"), wxPoint(10, 40), wxSize(45,
        20) );
    btnPause = new wxButton(this, ID_PAUSE, _("Pause"), wxPoint(53, 40), wxSize
        (45, 20) );
    btnStop = new wxButton(this, ID_STOP, _("Stop"), wxPoint(96, 40), wxSize(45,
        20) );

    btnClear = new wxButton(this, ID_CLEARANIM, _("Clear"), wxPoint(10, 64),
        wxSize(45, 20) );
    btnPrev = new wxButton(this, ID_PREVANIM, _T("<<"), wxPoint(53, 64), wxSize
        (45, 20) );
    btnNext = new wxButton(this, ID_NEXTANIM, _T(">>"), wxPoint(96, 64), wxSize
        (45, 20) );

    lockAnims = new wxCheckBox(this, ID_ANIM_LOCK, _("Lock Animations"),
        wxPoint(150, 40), wxDefaultSize, 0);
    lockAnims->SetValue(true);
    bLockAnims = true;
    oldStyle = new wxCheckBox(this, ID_OLDSTYLE, _("Auto Animate"), wxPoint(150,
        60), wxDefaultSize, 0);
    bOldStyle = true;
...
void AnimControl::UpdateModel(Model *m)
{
    ...
    {
        // only display the "secondary" animation list if its a character
    ...
    if(g_selModel->animated && g_selModel->anims)
    {
        ...
        for(unsigned int i = 0; i < g_selModel->header.nAnimations; i++)
        {
            try
            {
                AnimDB::Record rec = animdb.getByAnimID(g_selModel
                    ->anims[i].animID);
        ...
        frameSlider->SetRange(g_selModel->anims[useanim].timeStart, g_selModel
            ->anims[useanim].timeEnd);
        ...
        for(size_t i = 1; i < 4; i++)
        {
            ...
            if(NextAnimation >= 0)
            {
                g_selModel->animManager->addAnim(NextAnimation, loopList
                    ->GetSelection() );
            }
            else
            {
...
void AnimControl::UpdateWMO(WMO *w, int group)
{
    ...
    if(newwmo)
    {
        ...
        {
            wmoList->Append(wxString(g_selWMO->doodadsets[i].name, 
                *wxConvCurrent) );
        }
...
bool AnimControl::UpdateCreatureModel(Model *m)
{
    ...
    try
    {
        ...
        if(rec.getUInt(CreatureModelDB::Type) != 4)
        {
            ...
            for(CreatureSkinDB::Iterator it = skindb.begin(); it != skindb.end()
                ; ++it)
            {
    ...
    if(event.GetId() == ID_ANIM)
    {
        if(g_selModel)
        {
            ...
            if(bOldStyle == true)
            {
                ...
                g_selModel->animManager->Stop();
                g_selModel->animManager->Set(0, selectedAnim, loopList
                    ->GetSelection() );
#if 0
                int NextAnimation = selectedAnim;
                for(size_t i = 1; i < 4; i++)
                {
                    NextAnimation = g_selModel
                        ->anims[NextAnimation].NextAnimation;
                    if(NextAnimation >= 0)
                    {
                        g_selModel->animManager->addAnim(NextAnimation,
                            loopList->GetSelection() );
                    }
                    else
                    {
                        break;
                ...
                frameSlider->SetRange(g_selModel->anims[selectedAnim].timeStart,
                    g_selModel->anims[selectedAnim].timeEnd);
                frameSlider->SetTickFreq(g_selModel
                    ->anims[selectedAnim].playSpeed, 1);
            }
        }
...
void AnimControl::SetAnimFrame(int frame)
{
    ...
    int frameNum = (frame - g_selModel->anims[g_selModel
        ->currentAnim].timeStart);
