...
DebugPacket::Write(ushort portNum, uint packetCount, const ubyte* buf, SizeT numBytes)
{
    ...
    uint* uintHeader = (uint*) this->buffer;
    ushort* ushortHeader = (ushort*) this->buffer;
    ubyte* payloadPtr = this->buffer + HeaderSize;
    
    SizeT numActualBytes = Math::n_min(numBytes, MaxPayloadSize);
    ...
    else
    {
        ...
        uintHeader[1]   = byteOrder.Convert<uint>(packetCount);
        ushortHeader[4] = byteOrder.Convert<ushort>(portNum);
        ushortHeader[5] = byteOrder.Convert<ushort>(numActualBytes);
    }
...
DebugPacket::GetMagic() const
{
    ...
    uint* uintHeader = (uint*) this->buffer;
    FourCC magic = byteOrder.Convert<uint>(uintHeader[0]);
    return magic;
}
...
DebugPacket::GetCount() const
{
    ...
    ByteOrder byteOrder(ByteOrder::Network, ByteOrder::Host);
    uint* uintHeader = (uint*) this->buffer;
    uint count = byteOrder.Convert<uint>(uintHeader[1]);
    return count;
}
...
DebugPacket::GetPort() const
{
    ...
    ByteOrder byteOrder(ByteOrder::Network, ByteOrder::Host);
    ushort* ushortHeader = (ushort*) this->buffer;
    ushort portNum = byteOrder.Convert<ushort>(ushortHeader[4]);
    return portNum;
}
...
DebugPacket::IsFinalPacket() const
{
    ...
    ByteOrder byteOrder(ByteOrder::Network, ByteOrder::Host);
    ushort* ushortHeader = (ushort*) this->buffer;
    ushort payloadSize = byteOrder.Convert<ushort>(ushortHeader[5]);
