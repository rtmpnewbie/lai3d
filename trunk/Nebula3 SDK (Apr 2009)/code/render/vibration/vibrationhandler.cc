...
/**
*/
VibrationHandler::VibrationHandler() : targetVibratos(MaxPlayerCount),
                                       curHiFreqValues(MaxPlayerCount),
                                       curLowFreqValues(MaxPlayerCount),
                                       vibratosRunnung(MaxPlayerCount),
                                       reseted(false),
                                       lastTriggerTime(0)
{
    IndexT indeX;
    for (indeX = 0; indeX < MaxPlayerCount; indeX++)
    {
...
VibrationHandler::DoWork()
{
    ...
    {
        IndexT playerIdx;
        for (playerIdx = 0; playerIdx < MaxPlayerCount; playerIdx++)
        {
...
VibrationHandler::ApplyValues()
{
    ...
    for (playerIdx = 0; playerIdx < MaxPlayerCount; playerIdx++)
    {
        ...
        vib.wLeftMotorSpeed  = (WORD) (this->curLowFreqValues[playerIdx] * 65535.0f);
        vib.wRightMotorSpeed = (WORD) (this->curHiFreqValues[playerIdx] * 65535.0f);

        //n_printf("---> player %i, hi %f, low %f\n", playerIdx, this->curHiFreqValues[playerIdx], this->curLowFreqValues[playerIdx]);

        XInputSetState(playerIdx, &vib);
    }
}
