...
SM30LightServer::Open()
{
    ...
    String lightTexPath("tex:lighting/lightcones");
    lightTexPath.Append(TEXTURE_EXTENSION);
...
SM30LightServer::ApplyModelEntityLights(const Ptr<InternalModelEntity>& modelEntity)
{
    ...
    Array<Ptr<InternalGraphicsEntity> > localLights = modelEntity->GetLinks(InternalGraphicsEntity::LightLink);
	
    // do just, if there is at least one light
	SizeT numLocalLights = localLights.Size();
    if(numLocalLights > 0)
    {
        // check and remove globallight from locallight array    
        n_assert(localLights[0]->IsA(InternalGlobalLightEntity::RTTI));
        ...
        {    
            ...
            {       
                const Ptr<InternalAbstractLightEntity>& lightEntity = localLights[lightIndex].downcast<InternalAbstractLightEntity>();                    
        ...
        {  
            ...
            const Ptr<InternalAbstractLightEntity>& lightEntity = localLights[mappedIndex].downcast<InternalAbstractLightEntity>();
