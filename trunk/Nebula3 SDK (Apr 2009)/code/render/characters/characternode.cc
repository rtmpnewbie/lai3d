...
*/
Ptr<ModelNodeInstance>
CharacterNode::CreateNodeInstanceHierarchy(const Ptr<ModelInstance>& modelInst, const Ptr<ModelNodeInstance>& parentNodeInst)
{
...
CharacterNode::OnResourcesLoaded()
{
    ...
    // FIXME: as long as anim resource file doesn't provide clip names,
    // we need to set them up manually
    const Ptr<AnimResource> animRes = this->managedAnimResource->GetAnimResource();
    animRes->SetupClipNames(this->clipNames);
...
CharacterNode::ParseDataTag(const FourCC& fourCC, const Ptr<BinaryReader>& reader)
{
    ...
    else if (FourCC('CLIP') == fourCC)
    {
        // ClipNames
        n_assert(this->clipNames.IsEmpty());
        SizeT numClips = reader->ReadInt();        
        this->clipNames.Reserve(numClips);
        IndexT i;
        for (i = 0; i < numClips; i++)
        {
            this->clipNames.Append(reader->ReadString());
        }
    }
