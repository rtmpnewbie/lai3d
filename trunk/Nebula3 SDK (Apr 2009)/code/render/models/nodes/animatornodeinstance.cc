...
AnimatorNodeInstance::Setup(const Ptr<ModelInstance>& inst, const Ptr<ModelNode>& node, const Ptr<ModelNodeInstance>& parentNodeInst)
{
    ...
    for (animSecIdx = 0; animSecIdx < numAnimSections; animSecIdx++)
    {
        ...
        for (i = 0; i < animPaths.Size(); i++)
        {
            ...
            {
                Ptr<StateNodeInstance> stateNode = modelNodeInst.cast<StateNodeInstance>();
                if (varSem.IsValid())
                {
                    if (!stateNode->HasShaderVariableInstance(varSem))
                    {
                        newNode.shaderVar = stateNode->CreateShaderVariableInstance(varSem);                    
                    }
                    else
                    {
                        newNode.shaderVar = stateNode->GetShaderVariableInstance(varSem);
                    }
                }
            }
            ...
            {
                n_assert(newNode.shaderVar.isvalid());
            }
...
*/
void 
AnimatorNodeInstance::Update(Timing::Time time)
{
    ModelNodeInstance::Update(time);
    this->Animate(time);    
}
    ...
    for (animSecIdx = 0; animSecIdx < numAnimSections; animSecIdx++)
    {
        ...
        for (i = 0; i < curAnimSection.Size(); i++)
        {
            ...
            if (animNodeType == AnimatorNode::Float4Animator)
            {
                ...
                {
                    curNode.shaderVar->SetFloat4(key.GetValue());
                }
            }
            else if (animNodeType == AnimatorNode::FloatAnimator)
            {
                ...
                {
                    curNode.shaderVar->SetFloat(key.GetValue());
                    //n_printf("Float aniamation of node %s: time: %f value: %f\n", curNode.node->GetName().Value().AsCharPtr(), time, key.GetValue());                    
                }
            }
            else if (animNodeType == AnimatorNode::IntAnimator)
            {
                ...
                {
                    curNode.shaderVar->SetInt(key.GetValue());
                }
            }
            ...
            else if (animNodeType == AnimatorNode::TransformCurveAnimator)
            {
                ...
                AnimSampler::Sample(animResource, clipIndex, SampleType::Linear, sampleTime, result);
            ...
            else if (animNodeType == AnimatorNode::UvAnimator)
            {
                ...
                curNode.shaderVar->SetMatrix(uvTransform);
            }
        }
    }
}
