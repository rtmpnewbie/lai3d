...
namespace Attr
{
    ...
    // cam shake stuff
    DefineFloat(Intensity, 'aein', ReadOnly);
    DefineFloat(Range, 'aera', ReadOnly);

    // sound stuff
    DefineString(CueName, 'aecn', ReadOnly);
...
namespace Animation
{
__ImplementClass(Animation::AnimEventRegistry, 'AERE', Core::RefCounted);
__ImplementInterfaceSingleton(Animation::AnimEventRegistry);
...
AnimEventRegistry::LoadCamShakeEvents(const Util::StringAtom& tableName)
{
    ...
    for (index = 0; index < numRows; index++)
    {
        ...
        float intensity = reader->GetFloat(Attr::Intensity);
        float range = reader->GetFloat(Attr::Range);

        if (0 == duration && 0 == intensity && 0 == range)
        {
            continue;
        }
...
AnimEventRegistry::LoadSoundEvents(const Util::StringAtom& tableName)
{
    ...
    for (index = 0; index < numRows; index++)
    {
        ...
        Util::String id = reader->GetString(Attr::AnimEventName);
        Util::String cueName = reader->GetString(Attr::CueName);
        ...
        this->animEventSounds.Add(id, cueName);
    }
