...
namespace Animation
{
__ImplementClass(Animation::AnimEventTimingHandler, 'AETH', Animation::AnimEventHandlerBase);
...
AnimEventTimingHandler::OnFrame(Timing::Time time)
{
    ...
    if (this->curTimingActive)
    {
        ...
        Ptr<SetGlobalVariable> setAudio = SetGlobalVariable::Create();
        setAudio->SetVariableName(Util::StringAtom("GlobalPitch"));
        setAudio->SetVaribaleValue(this->curFactor * 100);
        AudioInterface::Instance()->Send(setAudio.cast<Messaging::Message>());
    }
}
} // namespace Animation
