...
GrassRenderer::SetupGrassLayerFromMultilayerNode(const Ptr<InternalGraphics::InternalModelEntity> gfxEntity, const Util::StringAtom& attrName, const Util::StringAtom& attrValue)
{
    ...
    Util::Array<String> tokens = attrValue.Value().Tokenize(" \t\n=", '"');
    ...
    if (!lib.IsValid())
    {
        n_error("GrassRenderer: no grass library set on grass layer in '%s'!", gfxEntity->GetResourceId().Value().AsCharPtr());
        return;
    }
