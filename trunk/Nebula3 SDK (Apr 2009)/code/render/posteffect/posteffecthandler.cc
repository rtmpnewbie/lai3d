...
*/
void
PostEffectHandler::Close()
{
    if (this->postEffectServer.isvalid())
    {
...
{
    // create post effect server
    this->postEffectServer = PostEffect::PostEffectServer::Create();
    this->postEffectServer->Open();
}
...
{       
    ...
    {
        Timing::Time time = Timing::SlaveTime::Instance()->GetTime(Timing::TimingType::ScaledTime);
        PostEffectServer::Instance()->OnFrame(time);
