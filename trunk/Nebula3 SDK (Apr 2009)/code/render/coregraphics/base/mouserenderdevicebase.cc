...
MouseRenderDeviceBase::Discard()
{
    ...
    for (i = 0; i < this->textures.Size(); i++)
    {
        ...
        if (resServer->HasSharedResource(tex->GetResourceId()))
        {
            resServer->UnregisterSharedResource(tex->GetResourceId());
        }
    }
    this->textures.Clear();
    this->isValid = false;
...
MouseRenderDeviceBase::PreloadTextures(const Array<ResourceId>& resIds)
{
    ...
    for (i = 0; i < resIds.Size(); i++)
    {
        ...
        {
            Ptr<Texture> tex = resServer->CreateSharedResource(resId, Texture::RTTI).downcast<Texture>();
            if (tex->IsLoaded())
            {
