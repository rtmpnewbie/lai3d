            ...
            for (i = 0; i < shaders.Size(); i++)
            {
                ...
                htmlWriter->Begin(HtmlElement::TableRow);
                    htmlWriter->Begin(HtmlElement::TableData);
                        htmlWriter->AddAttr("href", "/shader?shaderinfo=" + shd->GetResourceId().Value());
...
ShaderPageHandler::WriteShaderVariableTable(const Ptr<HtmlPageWriter>& htmlWriter, const Array<Ptr<ShaderVariable> >& vars)
{
    ...
    for (i = 0; i < vars.Size(); i++)
    {
        ...
        htmlWriter->Begin(HtmlElement::TableRow);
            ...
            htmlWriter->Element(HtmlElement::TableData, var->IsArray() ? "yes" : "no");
            htmlWriter->Element(HtmlElement::TableData, String::FromInt(var->GetNumArrayElements()));
        htmlWriter->End(HtmlElement::TableRow);
    }
    htmlWriter->End(HtmlElement::Table);
}
