...
TimeManager::OnActivate()
{
    ...
    this->time = Timing::MasterTime::Instance()->GetTicks();
    this->frameTime = 0;
...
TimeManager::Update()
{
    ...
    Tick curTime = Timing::MasterTime::Instance()->GetTicks();
...
*/
void
TimeManager::SetTimeFactor(float f)
{
    n_assert(f > 0);
    this->timeFactor = f;
}
...
TimeManager::StartTimeEffect(float timeFactor, Tick duration, Tick fadeIn, Tick fadeOut)
{
    ...
    Animation::AnimEventRegistry::TimingEvent event;
    event.attack = fadeIn;
...
TimeManager::ApplyTimeEffect()
{
    ...
    // set audio stuff to
    Audio::AudioPlayer::Instance()->SetGlobalVariable(Util::StringAtom("GlobalPitch"), this->timeFactor * 100);
}
}; // namespace Game
