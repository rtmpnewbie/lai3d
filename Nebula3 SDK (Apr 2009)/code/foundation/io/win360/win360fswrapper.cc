...
Win360FSWrapper::OpenFile(const String& path, Stream::AccessMode accessMode, Stream::AccessPattern accessPattern, DWORD flagsAndAttributes)
{
    ...
    Handle handle = CreateFile(nativePath.AsCharPtr(),  // lpFileName
...
Win360FSWrapper::DeleteDirectory(const String& path)
{
    n_assert(path.IsValid());
        ...
        const String& nativePath = path;
    #endif
    return (0 != ::RemoveDirectory(nativePath.AsCharPtr()));
}
...
Win360FSWrapper::FileExists(const String& path)
{
    n_assert(path.IsValid());
        ...
        const String& nativePath = path;
    #endif
    DWORD fileAttrs = GetFileAttributes(nativePath.AsCharPtr());
...
Win360FSWrapper::SetFileWriteTime(const String& path, FileTime fileTime)
{
    n_assert(path.IsValid());
        ...
        const String& nativePath = path;
    #endif
    Handle h = Win360FSWrapper::OpenFile(nativePath, Stream::ReadWriteAccess, Stream::Sequential);
    if (0 != h)
    {
...
Win360FSWrapper::GetFileWriteTime(const String& path)
{
    ...
    #endif
    FileTime fileTime;
    Handle h = Win360FSWrapper::OpenFile(nativePath, Stream::ReadAccess, Stream::Sequential);
    if (0 != h)
    {
    ...
    {
        n_error("Win360FSWrapper::GetFileWriteTime(): failed to open file '%s'!", path.AsCharPtr());
    }
    return fileTime;
}
...
Win360FSWrapper::CreateDirectory(const String& path)
{
    n_assert(path.IsValid());
        ...
        const String& nativePath = path;
    #endif
    return (0 != ::CreateDirectory(nativePath.AsCharPtr(), NULL));
}
