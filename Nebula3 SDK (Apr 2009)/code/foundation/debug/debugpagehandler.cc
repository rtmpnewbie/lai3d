    ...
    if (htmlWriter->Open())
    {
        ...
        htmlWriter->Begin(HtmlElement::Table);
            ...
            for (idx = 0; idx < debugTimers.Size(); ++idx)
            {
                ...
                {
            	    sortedTimer.Add(Variant(debugTimers[idx]->GetName().Value()), debugTimers[idx]);
                }
                else if (this->sortByColumn == "Min")
                {
                    sortedTimer.Add(Variant((float)minTime), debugTimers[idx]);
                }
                else if (this->sortByColumn == "Max")
                {
                    sortedTimer.Add(Variant((float)maxTime), debugTimers[idx]);
                }
                else if (this->sortByColumn == "Avg")
                {
                    sortedTimer.Add(Variant((float)avgTime), debugTimers[idx]);
                }
            }
            ...
            for (i = 0; i < debugTimers.Size(); i++)
            {
                ...
                htmlWriter->Begin(HtmlElement::TableRow);
                    htmlWriter->Begin(HtmlElement::TableData);
                        htmlWriter->AddAttr("href", "/debug?timer=" + name.Value());
        ...
        htmlWriter->Begin(HtmlElement::Table);
            ...
            for (i = 0; i < debugCounters.Size(); i++)
            {
                ...
                htmlWriter->Begin(HtmlElement::TableRow);
                    htmlWriter->Begin(HtmlElement::TableData);
                        htmlWriter->AddAttr("href", "/debug?counter=" + name.Value());
                        htmlWriter->Element(HtmlElement::Anchor, name.Value());
                    htmlWriter->End(HtmlElement::TableData);
