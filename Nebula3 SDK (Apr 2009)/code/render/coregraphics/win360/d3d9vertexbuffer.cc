...
*/
Math::bbox 
D3D9VertexBuffer::ComputeBoundingBox()
{
    Math::bbox result;
    BYTE* vertexPtr = (BYTE*)this->Map(MapRead);
    IndexT i;
    for (i = 0; i < this->numVertices ; i++)
    {
        float* coord = (float*)vertexPtr;
        result.extend(Math::point(coord[0], coord[1], coord[2]));
        coord += this->vertexLayout->GetVertexByteSize();
    }
    this->Unmap();
