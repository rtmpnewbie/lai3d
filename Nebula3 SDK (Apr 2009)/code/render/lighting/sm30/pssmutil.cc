...
//------------------------------------------------------------------------------
#include "stdneb.h"
#include "lighting/sm30/pssmutil.h"
...
PSSMUtil::CalculateSplitDistances()
{
    ...
    float zNear = this->cameraEntity->GetZNear();
...
PSSMUtil::CalculateFrustumPoints(IndexT splitIndex)
{
    ...
    float cameraNormWidth  = this->cameraEntity->GetNearWidth() / this->cameraEntity->GetZNear();
    float cameraNormHeight = this->cameraEntity->GetNearHeight() / this->cameraEntity->GetZNear();
    ...
    // scale frustum points outwards to avoid edge poblems
...
PSSMUtil::CalculateTransforms(IndexT splitIndex)
{
    ...
    for (i = 0; i < NumCorners; i++)
    {
        float4 p = float4::transform(this->frustumCorners[i], lightView);
    ...
    this->splitLightTransforms[splitIndex]     = lightView;
    this->splitProjTransforms[splitIndex]      = lightProj;
    this->splitLightProjTransforms[splitIndex] = lightViewProj;
}
