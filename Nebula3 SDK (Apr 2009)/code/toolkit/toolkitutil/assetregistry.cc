...
AssetRegistry::LoadRegistryFile(Logger& logger, const URI& uri)
{
    ...
    if (reader->Open())
    {
        ...
        for (i = 0; i < numItems; i++)
        {
            ...
            timeStamp.time.dwHighDateTime = reader->ReadUInt();
            timeStamp.time.dwLowDateTime = reader->ReadUInt();
...
AssetRegistry::SaveRegistryFile(Logger& logger, const URI& uri)
{
    ...
    if (writer->Open())
    {
        ...
        for (i = 0; i < this->registry.Size(); i++)
        {
            ...
            writer->WriteUInt(assetFile.GetTimeStamp().time.dwHighDateTime);
            writer->WriteUInt(assetFile.GetTimeStamp().time.dwLowDateTime);
