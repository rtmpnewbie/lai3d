...
AssetUpdater::Update(Logger& logger, const AssetRegistry& diffSet)
{
    ...
    for (i = 0; i < diffSet.GetNumAssets(); i++)
    {
        ...
        URI localFileUri(this->localRootPath + curEntry.GetPath().Value());
        URI localDirUri(this->localRootPath + curEntry.GetPath().Value().ExtractDirName());
        URI remoteFileUri(this->remoteRootPath + curEntry.GetPath().Value());
