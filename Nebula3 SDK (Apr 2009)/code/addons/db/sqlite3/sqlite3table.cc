...
Sqlite3Table::CompileInsertCommand()
{
    ...
    // compile the command
    bool compiled = this->insertCommand->Compile(this->database, sql);
    if (!compiled)
    {
...
Sqlite3Table::BindValueToCommand(const Ptr<Command>& cmd, IndexT wildCardIndex, IndexT valueTableColIndex, IndexT valueTableRowIndex)
{
    ...
    switch (this->valueTable->GetColumnValueType(valueTableColIndex))
    {
        ...
        case Attr::Float4Type:
            cmd->BindFloat4(wildCardIndex, this->valueTable->GetFloat4(valueTableColIndex, valueTableRowIndex));
            break;
        ...
        case Attr::Matrix44Type:
            cmd->BindMatrix44(wildCardIndex, this->valueTable->GetMatrix44(valueTableColIndex, valueTableRowIndex));
            break;
