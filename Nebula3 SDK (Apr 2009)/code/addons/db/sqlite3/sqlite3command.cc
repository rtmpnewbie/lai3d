...
Sqlite3Command::Compile(const Ptr<Database>& db, const Util::String& sqlCommand, ValueTable* resultTable)
{
    ...
    int err = sqlite3_prepare(sqliteHandle, this->sqlCommand.AsCharPtr(), -1, &this->sqliteStatement, &cmdTail);
    if (err != SQLITE_OK)
    {
        this->SetSqliteError();
        return false;
                ...
                case Attr::Float4Type:
                    {
						...
						byteOrder.ConvertInPlace(*ptr);
                        /**ptr = ByteOrder::ConvertFloat4(ByteOrder::LittleEndian, ByteOrder::Host, *ptr);*/
                        this->valueTable->SetFloat4(valueTableColumnIndex, rowIndex, *ptr);
                    #endif
                    }
                    break;
                ...
                case Attr::Matrix44Type:
                    {
                        ...
                        const void* ptr = sqlite3_column_blob(this->sqliteStatement, resultColumnIndex);
                        int size = sqlite3_column_bytes(this->sqliteStatement, resultColumnIndex);
                        n_assert(size == sizeof(Math::matrix44));
                        ...
                        #else
                        Math::matrix44 alignedVal;
                        alignedVal.loadu((scalar*)ptr);
                        byteOrder.ConvertInPlace<Math::matrix44>(alignedVal);
                        this->valueTable->SetMatrix44(valueTableColumnIndex, rowIndex, alignedVal);                                            
                        #endif                        
                    }                   
                    break;
...
*/
void
Sqlite3Command::BindFloat4(IndexT index, const float4& val)
{
    n_assert(0 != this->sqliteStatement);
    ...
    byteOrder.ConvertInPlace<Math::float4>(convertedVal);
...
//------------------------------------------------------------------------------
/**
    Bind a Math::matrix44 value by placeholder index. Either get the index from
    one of the IndexOf methods or use a 0-based index.
*/
void
Sqlite3Command::BindMatrix44(IndexT index, const Math::matrix44& val)
{
    n_assert(0 != this->sqliteStatement);
    ...
    byteOrder.ConvertInPlace<Math::matrix44>(convertedVal);
...
*/
void
Sqlite3Command::BindFloat4(const Util::String& name, const float4& val)
{
    Sqlite3Command::BindFloat4(Sqlite3Command::IndexOf(name), val);
}
...
*/
void
Sqlite3Command::BindFloat4(const Attr::AttrId& id, const float4& val)
{
    Sqlite3Command::BindFloat4(Sqlite3Command::IndexOf(id), val);
}
...
*/
void
Sqlite3Command::BindMatrix44(const Attr::AttrId& id, const Math::matrix44& val)
{
    Sqlite3Command::BindMatrix44(Sqlite3Command::IndexOf(id), val);
}
