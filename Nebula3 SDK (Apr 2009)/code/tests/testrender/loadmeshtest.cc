...
LoadMeshTest::Run()
{
    ...
    if (this->SetupRuntime())
    {
        ...
        const Atom<String> mesh1Name = "msh:cube.nvx2";
        const Atom<String> mesh2Name = "msh:pyramid.nvx2";
        this->Verify(!this->resServer->HasSharedResource(mesh1Name));
        this->Verify(!this->resServer->HasSharedResource(mesh2Name));
        
        Ptr<Mesh> mesh0 = this->resServer->CreateSharedResource(mesh1Name, Mesh::RTTI, (ResourceLoader*) StreamMeshLoader::Create()).downcast<Mesh>();
        ...
        if (mesh0->IsLoaded())
        {
            n_printf("Mesh '%s' sync-loaded in %d ticks.\n", mesh1Name.Value().AsCharPtr(), timer.GetTicks());
        ...
        Ptr<Mesh> mesh1 = this->resServer->CreateSharedResource(mesh2Name, Mesh::RTTI, (ResourceLoader*) StreamMeshLoader::Create()).downcast<Mesh>();
