...
*/
void
StateGraphicsProperty::SetupDefaultAttributes()
{
    GraphicsFeature::GraphicsProperty::SetupDefaultAttributes();
    ...
    {            
        ...
        if (this->curState.IsValid())
        {
            ...
            for (int i = 0; i < this->graphicsEntities.Size(); i++)
            {
                ...
                #if NEBULA3_DEBUG
                    n_printf("StateObject: Deactivate state %s \n", this->graphicsEntities[i]->GetResourceId().Value().AsCharPtr());
                #endif
                }
            }
        }
...
StateGraphicsProperty::OnLoadResources(const Util::Dictionary<Util::String, StateInfo>& resources)
{
    if (resources.Size() > 0)
    {
        ...
        {
            this->graphicsEntities.AppendArray(segGfxUtil.CreateAndSetupGraphicsEntities(resources.KeysAsArray()[i], worldMatrix));            
        }
