...
namespace Attr
{
    DefineString(State, 'STAT', ReadWrite);
    DefineMatrix44(StateBaseTransform, 'STBT', ReadWrite);
    DefineBoolWithDefault(StateBaseTransformSet, 'SBTS', ReadWrite, false);
    DefineString(StateFilename, 'STFN', ReadWrite);
}
...
{
    this->entity->AddString(Attr::State);
    this->entity->AddMatrix44(Attr::StateBaseTransform);
    this->entity->AddBool(Attr::StateBaseTransformSet);
    this->entity->AddFloat(Attr::EntityTriggerRadius);
    this->entity->AddString(Attr::StateFilename);
