...
CharHairGeosetsDB::Record CharHairGeosetsDB::getByParams(unsigned int race,
    unsigned int gender, unsigned int section)
{
    for(Iterator i = begin(); i != end(); ++i)
    {
        if(i->getUInt(Race) == race && i->getUInt(Gender) == gender && i
            ->getUInt(Section) == section)
        {
            return (*i);
        }
    }
...
int CharSectionsDB::getColorsFor(unsigned int race, unsigned int gender,
    unsigned int type, unsigned int section, unsigned int npc)
{
    int n = 0;
    for(Iterator i = begin(); i != end(); ++i)
    {
...
#ifndef WotLK
        if(i->getUInt(Race) == race && i->getUInt(Gender) == gender && i
            ->getUInt(Type) == type && i->getUInt(Section) == section && i
            ->getUInt(IsNPC) == npc)
        {
            n++;
        }
#else 
        if(i->getUInt(Race) == race && i->getUInt(Gender) == gender && i
            ->getUInt(Type) == type && i->getUInt(Section) == section)
        {
            n++;
        }
#endif 
...
int CharSectionsDB::getSectionsFor(unsigned int race, unsigned int gender,
    unsigned int type, unsigned int color, unsigned int npc)
{
    int n = 0;
    for(Iterator i = begin(); i != end(); ++i)
    {
#ifndef WotLK
        if(i->getUInt(Race) == race && i->getUInt(Gender) == gender && i
            ->getUInt(Type) == type && i->getUInt(Color) == color && i->getUInt
            (IsNPC) == npc)
        {
            n++;
        }
#else 
        if(i->getUInt(Race) == race && i->getUInt(Gender) == gender && i
            ->getUInt(Type) == type && i->getUInt(Color) == color)
        {
            n++;
        }
#endif 
...
CharSectionsDB::Record CharSectionsDB::getByParams(unsigned int race, unsigned
    int gender, unsigned int type, unsigned int section, unsigned int color,
    unsigned int npc)
{
    for(Iterator i = begin(); i != end(); ++i)
    {
        ...
        if(i->getUInt(Race) == race && i->getUInt(Gender) == gender && i
            ->getUInt(Type) == type && i->getUInt(Section) == section && i
            ->getUInt(Color) == color && i->getUInt(IsNPC) == npc)
        {
            return (*i);
        }
#else 
...
CharFacialHairDB::Record CharFacialHairDB::getByParams(unsigned int race,
    unsigned int gender, unsigned int style)
{
    for(Iterator i = begin(); i != end(); ++i)
    {
        if(i->getUInt(Race) == race && i->getUInt(Gender) == gender && i
...
{
    "All", "Helmets", "Neck", "Shoulder armor", "Shirts", "Chest armor", 
        "Belts", "Pants", "Boots", "Bracers", "Gloves", "Rings", "Accessories",
        "Daggers", "Shields", "Bows", "Capes", "Two-handed weapons", "Quivers",
        "Tabards", "Robes", "One-handed weapons", "Claws", "Offhand", "?",  
        // unused?
    "Thrown", "Guns and wands"
};
...
{
    sscanf(line, "%u,%u,%u,%u,%u,%u,%u", &id, &model, &itemclass, &subclass,
        &type, &sheath, &quality);
...
void ItemDatabase::cleanup(ItemDisplayDB &itemdb)
{
    ...
    {
        bool keepItem = (items[i].type == 0) || (itemset.find(items[i].model) 
            != itemset.end() );
        if(keepItem)
        {
...
{
    for(std::vector < ItemRecord > ::iterator it = items.begin(); it !=
        items.end(); ++it)
        if(it->model == id)
        {
            return it->id;
...
{
    for(std::vector < NPCRecord > ::iterator it = npcs.begin(); it != npcs.end()
        ; ++it)
    {
        if(it->id == id)
        {
