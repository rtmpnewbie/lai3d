...
BEGIN_EVENT_TABLE(ModelBankControl, wxWindow)EVT_BUTTON(ID_MODELBANK_ADD,
    ModelBankControl::OnButton)EVT_BUTTON(ID_MODELBANK_REMOVE, ModelBankControl
    ::OnButton)EVT_BUTTON(ID_MODELBANK_DISPLAY, ModelBankControl::OnButton)
    END_EVENT_TABLE()
...
ModelBankControl::ModelBankControl(wxWindow *parent, wxWindowID id)
{
    ...
    if(Create(parent, id, wxDefaultPosition, wxSize(270, 280), 0, _T(
        "ModelBankControlFrame") ) == false)
    {
        wxLogMessage(_T(
            "GUI Error: Failed to create a window for our ModelBankControl.") );
        return ;
    }
    ...
    lblName = new wxStaticText(this, wxID_ANY, _("Name:"), wxPoint(5, 15),
        wxDefaultSize, 0);
    txtName = new wxTextCtrl(this, ID_MODELBANK_NAME, wxEmptyString, wxPoint(40,
        10), wxDefaultSize, 0, wxDefaultValidator);

    btnAdd = new wxButton(this, ID_MODELBANK_ADD, _T("Add"), wxPoint(200, 10),
        wxDefaultSize);
    btnRemove = new wxButton(this, ID_MODELBANK_REMOVE, _T("Remove"), wxPoint
        (200, 40), wxDefaultSize);
    btnDisplay = new wxButton(this, ID_MODELBANK_DISPLAY, _T("Display"),
        wxPoint(200, 70), wxDefaultSize);

    lstBank = new wxListBox(this, wxID_ANY, wxPoint(5, 40), wxSize(190, 250), 0,
        NULL, wxLB_SINGLE | wxLB_ALWAYS_SB);
...
void ModelBankControl::AddModel()
{
    ...
    if(cd.modelType == MT_NORMAL)
    {
        ...
        TextureGroup *grp = (TextureGroup*)g_animControl->skinList
            ->GetClientData(val);
...
void ModelBankControl::SaveList()
{
    ...
    {
        wxLogMessage(_T(
            "Error: Was unable to save the ModelBank data to the HDD.") );
        return ;
    }
